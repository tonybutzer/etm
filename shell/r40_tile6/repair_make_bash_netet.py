...
def build_year_hash(out, start_year, end_year, product):
    ...
    for year in years:
        ...
        if len(prod_list) >= 377:
            ...
            print('not greater than 377', year)
            chore_year = year
...
def build_actual_bash_file(range_list):
    ...
    for year_range in range_list:
        ...
        logname="etmv1_" + str(year_range) +'_' + product
        image_custom = 'etm_docker_image'
        image = 'tbutzer/' + image_custom
        cmd = 'docker run -i --name {} {} python3 api_etm.py -i {} -o {} -y {} {} dummy'.format(logname, image, in1, out, year_range, product)
        print(cmd)
...
build_actual_bash_file(chore_list)
